...
public:
	...
	void BallCollisionAndPhysics(float fElapsedTime) {
		std::vector<std::pair<sBall*, sBall*>> vCollidingBalls;
		...
		for (auto& ball : vBalls) {
			...
			if (ball.px < 0) ball.px += float(ScreenWidth()); // Making sure the balls remains in screen space
			...
			for (auto& target : vBalls) {
				if (ball.id != target.id) {
					if (DoCirclesOverlap(ball.px, ball.py, ball.radius, target.px, target.py, target.radius)) {
						vCollidingBalls.push_back({ &ball, &target }); // This is for the physics part
		...
		for (auto c : vCollidingBalls) {
			...
			float nx = (b2->px - b1->px) / fDistance; // Normal vector
			float ny = (b2->py - b1->py) / fDistance;

			float tx = -ny; // Tangent vector
			float ty = nx;

			float dpTan1 = b1->vx * tx + b1->vy * ty; // Dot product tangent
			float dpTan2 = b2->vx * tx + b2->vy * ty;

			float dpNorm1 = b1->vx * nx + b1->vy * ny; // Dot product normal
			float dpNorm2 = b2->vx * nx + b2->vy * ny;

			float m1 = (dpNorm1 * (b1->radius - b2->radius) + 2.0f * b2->radius * dpNorm2) / (b1->radius + b2->radius); // Conservation of momentum in 1D
